---
conflicts:
  effect_structured:
    damage_bonus:
    - source: null
      value: 0.1
    - source: https://expedition33.wiki.fextralife.com/Teamwork
      value: 0.10000000000000009
effect: 10% increased damage while all allies are alive (the term 'increased' implies
  that this boost is additive, when it is actually multiplicative) (Also gives an
  extra hidden 1.1x damage buff when equipped).
effect_structured:
  damage_bonus: 0.1
image_path: _assets/Pictos/teamwork.png
image_url: https://expedition33.wiki.fextralife.com/file/Expedition-33/3-pictos-equipment-clair-obscur-expedition-33-wiki-guide75px.png
lumina_points_cost: 5
name: Teamwork
sources:
- https://expedition33.wiki.fextralife.com/Teamwork
- https://expedition33.wiki.fextralife.com/Pictos
stats_granted:
  Defense: 0
  Health: 0
type: picto
---



---
base_damage: 2744
character: Sciel
name: Corderon
passives:
- effect: Curse self on battle start. Deal 50% more damage while Cursed.
  effect_structured:
    damage_bonus: 0.5
  name: Lvl. 4
- effect: 'Reset Curse duration when entering Twilight state. Lvl: 20 : Play again
    when entering Twilight state.'
  effect_structured: {}
  name: Lvl. 10
scaling_stat: Defense
sources:
- https://expedition33.wiki.fextralife.com/Corderon
- https://expedition33.wiki.fextralife.com/Weapons
type: weapon
---

**Element:** Dark
**Scaling:** Defense S, Luck A

**Passives**

- *Lvl. 4* — Curse self on battle start. Deal 50% more damage while Cursed.
- *Lvl. 10* — Reset Curse duration when entering Twilight state. Lvl: 20 : Play again when entering Twilight state.

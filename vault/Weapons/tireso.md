---
base_damage: 3713
character: Gustave
image_path: _assets/Weapons/tireso.png
image_url: https://expedition33.wiki.fextralife.com/file/Expedition-33/tireso-verso-gustave-weapon-expedition-33-wiki-guide-130px.png
name: Tireso
passives:
- effect: Gain 1 Rank on applying Mark.
  effect_structured: {}
  name: Lvl. 4
- effect: 'Mark an enemy on Base Attack. Lvl: 20 : Apply Powerless on Marking an enemy.'
  effect_structured: {}
  name: Lvl. 10
- effect: 'Mark an enemy on Base Attack. Lvl: 20 : Apply Powerless on Marking an enemy.'
  effect_structured:
    applies_buff: powerless
  name: Lvl. 10
scaling_stat: Vitality
sources:
- https://expedition33.wiki.fextralife.com/Tireso
- https://expedition33.wiki.fextralife.com/Weapons
type: weapon
---

**Element:** Earth
**Scaling:** Vitality S, Defense A

**Passives**

- *Lvl. 4* — Gain 1 Rank on applying Mark.
- *Lvl. 10* — Mark an enemy on Base Attack. Lvl: 20 : Apply Powerless on Marking an enemy.

---
base_damage: 3293
character: Maelle
name: Veremum
passives:
- effect: If Stanceless, Base Attack switches to Offensive Stance.
  effect_structured: {}
  name: Lvl. 4
- effect: 'Counterattacks apply Defenceless. Lvl: 20 : +50% Critical Chance while
    in Offensive Stance.'
  effect_structured: {}
  name: Lvl. 10
scaling_stat: Luck
sources:
- https://expedition33.wiki.fextralife.com/Veremum
- https://expedition33.wiki.fextralife.com/Weapons
type: weapon
---

**Element:** Physical
**Scaling:** Vitality A, Luck S

**Passives**

- *Lvl. 4* — If Stanceless, Base Attack switches to Offensive Stance.
- *Lvl. 10* — Counterattacks apply Defenceless. Lvl: 20 : +50% Critical Chance while in Offensive Stance.
